...
ex_binary_template = Template('SELECT _vid_, val_id, 1 violations '
                              ...
                              'WHERE  $join_rel.$object = t3.object '
                              '  AND  t3.attribute = \'$rv_attr\' '
                              '  AND EXISTS (SELECT $other_rel.$object '
                              '              FROM   "$init_table" AS $other_rel '
                              '              WHERE  $join_rel.$object!= $other_rel.$object'
                              '                AND  $orig_predicates '
                              '                AND  t3.rv_val $operation $rv_val)')
...
class DCFeaturizerFusion(Featurizer):
    def specific_setup(self):
        self.name = 'DCFeaturizerFusion'
        self.constraints = self.ds.constraints
        self.init_table_name = AuxTables.current_init.name
            ...
            if not is_binary:
                ...
                for idx, rel in enumerate(join_rel):
                    ...
                    query = binary_template.substitute(init_table=self.init_table_name,
                                                       ...
                                                       rv_attr=rv_attr,
                                                       operation=op,
                                                       rv_val=rv_val, object= self.ds.key)
                    ...
                    ex_query = ex_binary_template.substitute(init_table=self.init_table_name,
                                                             ...
                                                             operation=op,
                                                             rv_val=rv_val,
                                                             other_rel=other_rel[idx],object= self.ds.key)
