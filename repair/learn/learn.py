...
class RepairModel:
    ...
    def fit_model(self, X_train, Y_train, mask_train, X_valid, Y_pred, mask_valid, valid_idx):
        ...
        lr_sched = ReduceLROnPlateau(optimizer, 'min', verbose=False, eps=1e-6, patience=2)
        ...
        for i in tqdm(range(epochs)):
            ...
            Y_pred_valid = self.__forward_valid__(X_valid, mask_valid)
            valid_loss = loss.forward(Y_pred_valid, Variable(Y_pred, requires_grad=False).squeeze(1))
            logging.debug('overall training loss: %f', valid_loss)
    ...
    def __train__(self, loss, optimizer, X_train, Y_train, mask_train):
        ...
        optimizer.zero_grad()
        # Fully-connecte)d layer with shared parameters between output classes
    ...
    def __forward_valid__(self, X_pred, mask_pred):
        X_var = Variable(X_pred, requires_grad=False)
        index = torch.LongTensor(range(X_var.size()[0]))
        index_var = Variable(index, requires_grad=False)
        mask_var = Variable(mask_pred, requires_grad=False)
